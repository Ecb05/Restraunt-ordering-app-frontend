...
const TepanyakiPage = () => {
  ...
  useEffect(() => {
    const loadDishes = async () => {
      try {
        const data = await fetchData("tepanyaki");
        setDishes(data);
      } catch (error) {
        console.error("Error fetching Tepanyaki data:", error);
      }
    };
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col pb-20">
      ...
      <div className="flex space-x-3 p-4 overflow-x-auto">
        {["All", "Beef", "Chicken", "Fish", "Veg"].map((category) => (
          <button
            key={category}
